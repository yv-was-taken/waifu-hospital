...
app.get('/api/characters/:id', async (req, res) => {
  ...
  try {
    ...
    logger.error(`Failed to fetch character ${id} from backend:`, error.message);
...
async function fetchCharacters() {
  try {
    ...
    if (publicResponse.data && Array.isArray(publicResponse.data)) {
      logger.info(`Fetched ${publicResponse.data.length} public characters from backend`);
    ...
    if (allCharacters.length > 0) {
      ...
      logger.info(`Set ${allCharacters.length} total characters in the character cache`);
    } else {
