...
class ChatService {
  ...
  async generateResponse(characterId, userMessage) {
    ...
    if (!character) {
      try {
        console.log(`Character ${characterId} not found in cache, fetching from backend...`);
        ...
        const response = await axios.get(`${backendUrl}/api/characters/${characterId}`);
        ...
        console.error(`Failed to fetch character ${characterId} from backend:`, error.message);
        throw new Error(`Character with ID ${characterId} not found`);
      }
    }
    
    if (!character) {
      throw new Error(`Character with ID ${characterId} not found after fetch attempt`);
    }

    try {
      // Generate the system prompt from character attributes
      const systemPrompt = character.getSystemPrompt();
      console.log(`Generated system prompt for ${character.name}:`, systemPrompt);
      ...
      try {
        ...
        if (apiError.response) {
          ...
          console.error("Authentication error with OpenAI API. Check your API key:", apiError.message);
        } else {
  ...
  getFallbackResponse(characterId) {
    ...
    if (character) {
      ...
      return `Hi there! I'm ${character.name}${character.occupation ? `, ${character.occupation}` : ''}. ${
        character.personality ? `I'm known for being ${character.personality}.` : ''
      } ${
