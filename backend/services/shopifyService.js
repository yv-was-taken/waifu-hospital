...
const createProduct = async (merchandiseData) => {
  try {
    ...
    if (!process.env.SHOPIFY_API_KEY || process.env.SHOPIFY_API_KEY === 'dummy_key') {
      console.warn('Shopify integration not properly configured. Returning mock product data.');
      return {
        id: `mock_${Date.now()}`,
    ...
    const session = await shopify.session.customAppSession(process.env.SHOPIFY_SHOP || 'dummy-shop.myshopify.com');
...
const generateVariants = (merchandiseData) => {
  ...
  const sizes = merchandiseData.sizes?.length ? merchandiseData.sizes : ['Default'];
  const colors = merchandiseData.colors?.length ? merchandiseData.colors : ['Default'];
...
const createCheckout = async (lineItems) => {
  try {
    // Check if Shopify API key is set, if not return mock data
    if (!process.env.SHOPIFY_API_KEY || process.env.SHOPIFY_API_KEY === 'dummy_key') {
      console.warn('Shopify integration not properly configured. Returning mock checkout data.');
      return {
